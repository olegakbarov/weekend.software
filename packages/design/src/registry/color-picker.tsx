...
function ChannelSlider({
  ...
  return (
    <div
      ...
      onKeyDown={onKeyDown}
      className="relative w-full h-3 cursor-pointer select-none touch-none rounded-full outline-none focus-visible:ring-1 focus-visible:ring-[#6B97FF]"
      style={trackStyle}
    >
      <div
        className="absolute pointer-events-none rounded-full"
...
function FormatDropdown({
  ...
  const [pos, setPos] = useState<
    | { mode: "fixed" | "absolute"; top: number; left: number; width: number }
    | null
  >(null);
...
// ---------------------------------------------------------------------------
// ColorInput (channel scrubber + text editor)
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// EyeDropperButton — uses browser EyeDropper API.
// ---------------------------------------------------------------------------
...
function EyeDropperButton({ onPick }: { onPick: (hex: string) => void }) {
  ...
  const handleClick = async () => {
    try {
      const Ctor = (window as unknown as { EyeDropper: new () => EyeDropperGlobal })
        .EyeDropper;
      const eye = new Ctor();
  ...
  (
    ...
    return (
      <div
        ref={ref}
        className={cn(
          "flex flex-col gap-2 p-3 bg-card border border-border/60",
          shape.container,
          className,
        )}
