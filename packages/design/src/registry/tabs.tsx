...
/**
 * Tabs — animated segmented tab control.
 *
 * Sits on a `bg-muted` tray; the active item is indicated by an animated
 * `bg-background` pill that springs between triggers. Hover and keyboard focus
 * each get their own animated overlay rect (proximity hover follows the
 * pointer; focus ring inflates 2px around the focused tab). Labels use an
 * inline-grid double-render trick so the semibold-active weight doesn't shift
 * neighbouring tabs.
 *
 * Compound API:
 ...
 *   </Tabs>
 *
 * Upstream parity: ports `registry/default/tabs.tsx` from
 * mickadesign/fluid-functionalism. Uses Phase F's `springs`, `useShape`,
 * `useProximityHover({ axis: "x" })` foundations.
 *
 * Back-compat: `TabsTrigger` and `TabsContent` are kept as deprecated aliases
 * for `TabItem` and `TabPanel` respectively (see bottom of this file).
 */
...
const Tabs = forwardRef<HTMLDivElement, TabsProps>(
  (
    ...
    // Conditionally spread value/defaultValue so exactOptionalPropertyTypes
    // does not see `undefined` flowing into Radix's `value: string` slot.
      ...
      >
        ...
        >
          ...
          {selectedRect && (
            <motion.div
              className={cn(
                "absolute pointer-events-none bg-[var(--surface-raised)] shadow-sm dark:shadow-[0_1px_2px_rgba(0,0,0,0.4)]",
                shape.bg,
              )}
              initial={false}
              animate={{
...
/**
 * @deprecated Use `TabItem` instead. `TabsTrigger` is a thin back-compat wrapper
 * that maps `children` → `TabItem`'s `label` prop (string-coerced).
 */
...
/**
 * @deprecated Use `TabItem`. Kept for back-compat with earlier Weekend call
 * sites that imported `TabsTrigger` (Radix-style). `children` is coerced to a
 * string and forwarded as `label` so the inline-grid bold-ghost trick still
 * works; pass a string for best results.
 */
const TabsTrigger = forwardRef<HTMLButtonElement, TabsTriggerProps>(
  ({ children, ...props }, ref) => {
    const label = typeof children === "string" ? children : String(children ?? "");
    return <TabItem ref={ref} label={label} {...props} />;
  },
);
TabsTrigger.displayName = "TabsTrigger";

/**
 * @deprecated Use `TabPanel`. Kept for back-compat with earlier Weekend call
 * sites that imported `TabsContent` (Radix-style).
 */
